...
public class ApproveTaskHandler implements TaskListener {
	...
	private static Logger logger = LoggerFactory.getLogger(ApproveTaskHandler.class);
	...
	public void notify(DelegateTask paramDelegateTask) {
		...
		try {
			...
			String approverGroupId = environment.getProperty("approver.group", "approver");
			String[] groupIds = approverGroupId.split(",");
			...
			String approverUser = environment.getProperty("approver.user", "");
			logger.debug("approverUser = " + approverUser);
			String[] userIds = approverUser.split(",");

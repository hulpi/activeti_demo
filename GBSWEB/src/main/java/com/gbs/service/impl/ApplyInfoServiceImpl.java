...
public class ApplyInfoServiceImpl implements ApplyInfoService {
	...
	public void addIndividualApplyInfo(APPLY_INFO_DTO dto,
			GU_INDIVIDUAL_INFO individual, UserInfoDto userDto)
			throws Exception {
	...
	@Transactional(rollbackFor = Exception.class)
	public void addEnterpriseApplyInfo(APPLY_INFO_DTO dto,
			GU_ENTERPRISE_INFO enterprise, UserInfoDto userDto)
			throws Exception {

		// 检查个人或企业 是否已经申请过担保，不能重复申请
		GU_APPLY_INFO applyInfoDb = applyInfoDao
				.queryApplyInfoForEnterprise(dto.getEn_id());
	...
	public void addApplyInfo(APPLY_INFO_DTO dto, GU_INDIVIDUAL_INFO individual,
			GU_ENTERPRISE_INFO enterprise, UserInfoDto userDto)
			throws Exception {
		...
		GU_CITY_INFO city = cityService.queryCityByOid(Long.valueOf(dto
				.getCity_id()));
		if (city != null) {
		...
		// 工作流引擎
		/*
		 * String process_instance_id = callWorkFlow(userId,
		 * applyInfo.getGuarant_id());
	...
	public String callWorkFlow(String userId, String businessKey)
			throws Exception {
	...
	public INDIVIDUAL_INFO_DTO getIndival(String guarant_id) {
		...
		if (individual.getCERT_NO() != null) {
			INDIVIDUAL_INFO_DTO individualInfo = individualInfoService
					.getIndividualInfo(individual.getCERT_NO());
	...
	public APPLY_INFO_DTO getApplyInfoByGuarantId(String guarantId) {
		GU_APPLY_INFO applyInfo = applyInfoDao
				.getApplyInfoByGuarantId(guarantId);
		APPLY_INFO_DTO applyInfoDto = new APPLY_INFO_DTO();
		if (applyInfo != null) {
			applyInfoDto.setGuarant_id(applyInfo.getGuarant_id());
			applyInfoDto.setGuarant_amt(String.valueOf(applyInfo
					.getGuarant_amt()));
			if (applyInfo.getCity() != null) {
				applyInfoDto.setCity_id(String.valueOf(applyInfo.getCity()
						.getGu_city_info_oid()));
			}

			if (applyInfo.getFunder() != null) {
				applyInfoDto.setFunder_id(String.valueOf(applyInfo.getFunder()
						.getGu_funder_info_oid()));
			}

			if (applyInfo.getProduct() != null) {
				applyInfoDto.setProductSel(String.valueOf(applyInfo
						.getProduct().getGu_product_info_oid()));
			}
			applyInfoDto.setProduct_type(applyInfo.getGuarant_type());

			applyInfoDto.setGuarant_time_limit(String.valueOf(applyInfo
					.getGuarant_time_limit()));
			...
			if (applyInfo.getEnterprise() != null) {
				applyInfoDto
						.setEn_id(applyInfo.getEnterprise().getREGISER_NO());
			}
	...
	public void updateApplyInfo(APPLY_INFO_DTO applyInfoDto) {
		GU_APPLY_INFO applyInfo = applyInfoDao
				.getApplyInfoByGuarantId(applyInfoDto.getGuarant_id());
		Date now = new Date();
		...
		if (applyInfoDto.getFunder_id() != null) {
			GU_FUNDER_INFO funder = new GU_FUNDER_INFO();
			funder.setGu_funder_info_oid(Long.valueOf(applyInfoDto
					.getFunder_id()));
			applyInfo.setFunder(funder);
		}
		...
		if (applyInfoDto.getGuarant_amt() != null) {
			applyInfo.setGuarant_amt(new BigDecimal(applyInfoDto
					.getGuarant_amt()));
		}
		if (applyInfoDto.getGuarant_time_limit() != null) {
			applyInfo.setGuarant_time_limit(Long.valueOf(applyInfoDto
					.getGuarant_time_limit()));
		}

		if (applyInfoDto.getProductSel() != null) {
			GU_PRODUCT_INFO product = new GU_PRODUCT_INFO();
			product.setGu_product_info_oid(Long.valueOf(applyInfoDto
					.getProductSel()));
			applyInfo.setProduct(product);
		}
	...
	public List<UiApplyInfoDto> getApplyInfoList(APPLY_INFO_DTO dto, Page page) {
		List<GU_APPLY_INFO> list = applyInfoDao.getApplyInfoList(dto, page);
		List<UiApplyInfoDto> resList = new ArrayList<UiApplyInfoDto>();
		DateFormat df = new java.text.SimpleDateFormat("yyyy-MM-dd");
		DecimalFormat def = new DecimalFormat(",###");
		if (list != null && list.size() > 0) {
			for (GU_APPLY_INFO applyInfo : list) {
				UiApplyInfoDto uiDto = new UiApplyInfoDto();
				uiDto.setGuarant_id(applyInfo.getGuarant_id());
				uiDto.setApply_date(df.format(applyInfo.getApply_date()));
				if (applyInfo.getIndividual() != null) {
					uiDto.setCustName(applyInfo.getIndividual().getCUST_NAME());
				}
				if (applyInfo.getEnterprise() != null) {
					uiDto.setCustName(applyInfo.getEnterprise()
							.getENTERPRISE_NAME());
				}
				if (applyInfo.getProduct() != null) {
					uiDto.setProductName(applyInfo.getProduct()
							.getProduct_desc());
				}
				if (applyInfo.getCity() != null) {
					uiDto.setCityName(applyInfo.getCity().getCity_name());
				}

				uiDto.setApply_status(ApplyStatus.getApplyStatusDes(applyInfo
						.getApply_status()));
				if (applyInfo.getGuarant_amt() != null) {
					uiDto.setGuarant_amt(def.format(applyInfo.getGuarant_amt()));
				}
				if (applyInfo.getApprove_amt() != null) {
					uiDto.setApprove_amt(def.format(applyInfo.getApprove_amt()));
				}
				uiDto.setGuarant_type(configService.getProductTypeDes(applyInfo
						.getGuarant_type()));
				uiDto.setGuarant_time_limit(applyInfo.getGuarant_time_limit());
				uiDto.setLoan_purpose(configService.getLoanPurposeDes(applyInfo
						.getLoan_purpose()));
				uiDto.setApplicant(applyInfo.getApplicant());
	...
	public int getApplyInfoCount(APPLY_INFO_DTO dto) {
		return applyInfoDao.getApplyInfoCount(dto);
	}
	...
	public void confirmApply(String guarant_id, String userid) throws Exception {
		...
		process_instance_id = callWorkFlow(userid, guarant_id);
		GU_APPLY_INFO applyInfo = applyInfoDao.getApplyInfoByGuarantId(guarant_id);
		applyInfo.setProcess_instance_id(process_instance_id);
		applyInfo.setApply_status(ApplyStatus.CONFIRM_APPLY);
		applyInfoDao.updateApplyInfo(applyInfo);
	}
